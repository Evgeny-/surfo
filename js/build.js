...
angular.module('angularMoment', [])
	.directive('amTimeAgo', ['$window', '$timeout', '$parse', function ($window, $timeout, $parse) {
		'use strict';
		...
		return function (scope, element, attr) {
         ...
         updateTime($window.moment(new Date($parse(attr.amTimeAgo)(scope))));
...
var APP_NAME = "CoteApp",
    SERVER_URL = "http://app.rankedgaming.ru/";// || "http://app.rankedgaming.ru/";

App = angular.module('App', ['hmTouchevents', 'angularMoment', 'ngResource', 'angularytics'], null);
...
App.controller('IndexController',
   ['$scope', '$location', 'Header', 'Menu',
   function ($scope, $location, Header, Menu) {
...
App.controller('PostsController',
   ['$scope', '$resource', 'store', 'zoom', 'Header', '$routeParams', 'Menu',
   function ($scope, $resource, store, zoom, Header, $routeParams, Menu) {
      ...
      $scope.openGif = function (url, ev) {
         var ga = ev.target.getAttribute.bind(ev.target),
            sa = ev.target.setAttribute.bind(ev.target),
            opened = ga('data-opened') != 'true';

         sa('src', ga(opened ? 'data-url' : 'data-preview'));
         sa('data-opened', opened);
      };
      ...
      function onScroll () {
         ...
         if((docHeight - windowHeight - scrollTop) < 600) {
            loadMore();
            $scope.$apply();
            canLoad = false;
         }
...
App.controller('ZoomController',
   ['$scope', '$rootScope', 'zoom', function ($scope, $rootScope, zoom) {
      ...
      var img = document.getElementById('zoom-image'),
          wrap = document.getElementById('zoom-wrap'),
          image = null, zoomed = false, top = 0, left = 0;
      ...
      $scope.drag = function (ev) {
         ...
         if(ev.gesture.touches.length !== 1) return;

         ev.preventDefault();
         ev.gesture.preventDefault();

         wrap.style.top  = top + ev.gesture.deltaY + 'px';
         wrap.style.left = left + ev.gesture.deltaX + 'px';
      };

      $rootScope.$on('zoom:close', function (e) {
         close();
      });

      $rootScope.$on('zoom:open', function (e, img) {
         image = img;
         zoomed = false;
         top = left = 0;
      ...
      angular.element(wrap).bind('dragend', function () {
         top = +wrap.style.top.replace('px', '');
         left = +wrap.style.left.replace('px', '');
      });
      ...
      function begin () {
         ...
         if(wh < image.height || ww < image.width) {
            var newHeight = wh - 20, newWidth = ww - 20;
...
angular.module("App").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("views/main.html",
    ...
    "   <div class=\"button-container\" ng-repeat=\"(id, menu) in menus\">\n" +
    "      <div class=\"button\" ng-style=\"{background: menu.color}\" hm-tap=\"openMenu($index, menu.submenu.length)\">\n" +
    "         {{ menu.name }}\n" +
    "      </div>\n" +
    "\n" +
    ...
    "      <div class=\"button favorite\" hm-tap=\"openFavorite()\">Избранное</div>\n" +
    "   </div>\n" +
    "\n" +
    "</div>\n" +
    "\n" +
    "<div id=\"bg-container\">\n" +
    "    <div class=\"image-bg image\" ng-class=\"{show: true}\"></div>\n" +
    "</div>"
  );
  ...
  $templateCache.put("views/posts.html",
    ...
    "               <span class=\"gif\"></span>\n" +
    "               <img ng-src=\"{{ gif.preview }}\" ng-click=\"openGif(gif.url, $event)\" data-url=\"{{ gif.url }}\" data-preview=\"{{ gif.preview }}\" data-opened=\"false\" class=\"image gif-image\">\n" +
    "            </div>\n" +
    "         </div>\n" +
    "\n" +
