...
(function() {
      ...
      $scope.download = function() {
        ...
        try {
          ...
          return fileTransfer.download($scope.image, "file:///sdcard/surfo/" + name, (function(entry) {
            return alert("Изображение сохранено в /surfo/" + name);
          }), function() {
            return alert("Ошибка сохранения изображения :(");
          });
        } catch (_error) {
          e = _error;
          return alert("Ошибка: " + e.toString());
        }
      };
  ...
  App.directive('menuTabletSize', [
    '$rootScope', function($rootScope) {
      return function(scope, el, attr) {
        var interval;
        interval = null;
        return $rootScope.$watch('TABLET', function(res) {
          if (!res) {
            return;
          }
          clearInterval(interval);
          return setTimeout((function() {
            var header, menu, setHeight;
            header = el[0].getElementsByClassName('header')[0];
            ...
            setHeight = function() {
              ...
              return menu.style.height = (window.innerHeight - header.clientHeight) + 'px';
            };
            window.addEventListener('resize', setHeight);
  ...
  App.directive('setBackground', [
    '$rootScope', 'Menu', '$routeParams', 'Color', function($rootScope, Menu, $routeParams, Color) {
      return function(scope, el, attr) {
        return $rootScope.$on('$routeChangeSuccess', function(event, next, current) {
          el[0].style.backgroundColor = null;
          return Menu.getHeaderMenu($routeParams.cat, function(menu) {
            var color;
            color = Color.lighten(menu.colors, 0.9);
            return el[0].style.backgroundColor = color;
          });
        });
      };
    }
  ...
  App.filter('dateago', function() {
    return function(time) {
      ...
      if (diff < 60) {
        ...
        return ~~(diff / 3600 / 24) + ' д назад';
      } else {
        return ~~(diff / 30 / 24 / 3600) + ' мес назад';
      }
    };
  });
  ...
  App.run([
    'Zoom', 'Share', function(Zoom, Share) {
      return document.addEventListener("backbutton", (function(e) {
        if (Zoom.opened() || Share.opened()) {
          Zoom.close();
          Share.close();
  ...
  App.service('Color', Color = (function() {
    function Color() {}

    Color.prototype.lighten = function(color, percent) {
      var B, G, R, f, p, t;
  ...
  App.service('Menu', [
    'Api', 'Store', Menu = (function() {
      ...
      Menu.prototype.callbacks = [];

      Menu.prototype.loading = false;
  ...
  App.service('Posts', [
    'Api', 'Like', Posts = (function() {
      ...
      Posts.prototype.LIMIT_SHOW = 8;

      Posts.prototype.MIN_CACHE = 20;
  ...
  App.service('Share', [
    '$rootScope', Share = (function() {
      ...
      Share.prototype.opened = function() {
  ...
  App.service('Zoom', [
    '$rootScope', Zoom = (function() {
      ...
      Zoom.prototype.opened = function() {
